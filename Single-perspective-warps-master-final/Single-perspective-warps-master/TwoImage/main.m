...
pathname = strcat('Images\2\'); %
outpath = strcat(pathname, 'results\');
imgs_format = '*.jpg'; 
dir_folder = dir(strcat(pathname, imgs_format));

% Display the contents of dir_folder to verify
% disp('Current working directory:');
% disp(pwd);

% % List all files and folders in the current directory
% disp('Files and folders in the current directory:');
% disp({dir().name});
...
clear V_star; % show the warped control vertices in axis
