...
disp("energyPointAlign function")
disp(size(points_xk));
disp(size(points_predict));
disp(size(lines_xk));
disp(size(lines_predict));
disp(ref_n);
disp(size(cell_imgs{ref_n}));
% fprintf('> COnverting to grayscale...');tic;
% for i = 1:length(cell_imgs)
%     if size(cell_imgs{i}, 3) == 3 % Check if the image is RGB
%         cell_imgs{i} = im2gray(cell_imgs{i}); % Convert to grayscale
%     end
% end
% fprintf('done (%fs)\n',toc);
...
if k > 1
    ...
    sparse_line_ref = sparse([], [], [], 0, 2*(num_imgs-1)*num_V);
end
